...
func Create(isEnabled bool, logFile string, debugMode bool) *zap.Logger {
	if !isEnabled || logFile == ":null" {
		return createDisabledLogger(debugMode)
	}
	core := createZapProductionCore(createWriteStream(logFile), debugMode)
	if debugMode {
...
func createWriteStream(logFile string) io.Writer {
	if logFile == "" {
		logDir := "logs/"
		defaultLog := logDir + meta.Package + ".log"
